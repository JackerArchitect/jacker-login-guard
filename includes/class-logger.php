...
class Logger {
	 ...
	 *
	 * Also checks whether the IP should be auto-blocked for repeated
	 * failures within the rolling window.
	 *
	 * @since 1.0.0
	...
	public function log_failed_login( $username ) {
		...
		$this->maybe_auto_block_after_failed_login();
	}
	...
	private function insert_login_event( $username, $status ) {
		...
		$username = self::truncate( $username, 255 );
		...
		$this->maybe_maintain_limit( $table );
	}
	...
	public function log_access_event( $target, $status = 'allowed', $reason = '' ) {
		...
		$target = self::truncate( $target, 255 );
		$reason = self::truncate( $reason, 255 );
		...
		$user_agent  = self::truncate( $user_agent, 1000 );
		$request_url = self::truncate( $request_url, 2000 );
		$referer     = self::truncate( $referer, 255 );
	...
	/**
	 * Count blocked requests for an IP within a time window.
	 *
	 * @since 1.0.0
	 * @param string $ip    IP address.
	 ...
	 * @return int
	 */
	private function count_blocked_requests( $ip, $since = 0 ) {
		global $wpdb;
		$table = Tables::get( JACKLOGU_ACCESS_TABLE );

		if ( $since > 0 ) {
			$since_str = gmdate( 'Y-m-d H:i:s', $since );

			// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$count = (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM {$table} WHERE user_ip = %s AND status = %s AND event_time > %s",
					$ip,
					'blocked',
					$since_str
				)
	...
	/**
	 * Count failed login attempts for an IP within a time window.
	 *
	 * @since 1.0.0
	 * @param string $ip    IP address.
	 * @param int    $since Unix timestamp.
	 * @return int
	 */
	private function count_failed_logins( $ip, $since ) {
		global $wpdb;
		$table = Tables::get( JACKLOGU_LOGIN_TABLE );
		...
		$count = (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$table} WHERE user_ip = %s AND status = %s AND login_time > %s",
				$ip,
				'failed',
	...
	public function auto_block_ip( $ip, $reason ) {
		...
		// Already blocked (active, unexpired)?
		// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$active = $wpdb->get_var(
			$wpdb->prepare(
		...
		if ( $last_block ) {
			...
			// First-time offenders use a rolling 24-hour window.
			$since_ts = time() - DAY_IN_SECONDS;
		}

		$attacks_since = $this->count_blocked_requests( $ip, $since_ts );
		...
		// Invalidate the "not blocked" cache for this IP.
		delete_transient( 'jacklogu_not_blocked_' . md5( $ip ) );

		$this->maybe_maintain_limit( $auto_block_table );

		$this->maybe_escalate_to_blacklist( $ip );
	}

	/**
	 * Auto-block an IP when its failed-login count crosses the threshold.
	 *
	 * Runs on every failed login. The threshold is configurable and can be
	 * disabled by setting it to 0.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	private function maybe_auto_block_after_failed_login() {
		if ( ! $this->settings->get( 'auto_block_attacks' ) ) {
			return;
		}

		$threshold = $this->settings->get_failed_login_block_threshold();
		...
		$count = $this->count_failed_logins( $ip, $since );
		...
		$reason = sprintf(
			__( 'Too many failed login attempts (%d within 24 hours)', 'jacker-login-guard' ),
			$count
		);

		$this->auto_block_ip( $ip, $reason );
	}
	...
	private function is_already_blacklisted( $ip ) {
		...
		foreach ( $lines as $line ) {
			...
			if ( IpUtils::ip_in_range( $ip, $range ) ) {
				return true;
			}
		}
	...
	private function add_ip_to_blacklist( $ip ) {
		...
		$current = isset( $fresh['blacklist_ips'] ) ? $fresh['blacklist_ips'] : '';
		$lines   = array_filter( array_map( 'trim', explode( "\n", $current ) ) );
	...
	private function remove_ip_from_whitelist( $ip ) {
		...
		$current = isset( $fresh['whitelist_ips'] ) ? $fresh['whitelist_ips'] : '';
		$lines   = array_filter( array_map( 'trim', explode( "\n", $current ) ) );
		$new     = array();
	...
	/**
	 * Run maintain_limit() at most once every N writes.
	 *
	 * @since 1.0.0
	 * @param string $table Fully-prefixed table name.
	 * @return void
	 */
	private function maybe_maintain_limit( $table ) {
		$key   = 'jacklogu_maint_' . md5( $table );
	...
	private function maintain_limit( $table ) {
		global $wpdb;
		$limit = (int) $this->settings->get_log_limit();
