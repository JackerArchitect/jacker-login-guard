 ...
 * Settings management.
 *
 * Loads, caches, and sanitizes the plugin's option array. All getters
 * clamp to safe ranges so callers can rely on the returned values.
 *
 ...
 * Class Settings
 *
 * Singleton wrapper around the plugin's option array.
 *
 * @since 1.0.0
 */
class Settings {
	...
	/**
	 * Migrate the legacy option key to the current one, if needed.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	...
	public function get_log_limit() {
		$limit = intval( $this->get( 'log_retention_limit', 300 ) );
	...
	public function get_session_lifetime() {
		$minutes = intval( $this->get( 'session_lifetime', 0 ) );
	...
	/**
	 * Get the auto-block trigger threshold (attacks before a block).
	 *
	 * @since 1.0.0
	 * @return int Always at least 1.
	 */
	public function get_auto_block_trigger_threshold() {
		$value = intval( $this->get( 'auto_block_trigger_threshold', 3 ) );
	...
	public function get_auto_block_duration_seconds() {
		$hours = intval( $this->get( 'auto_block_duration_hours', 24 ) );
	...
	/**
	 * Get the auto-block → blacklist escalation threshold.
	 *
	 * @since 1.0.0
	 * @return int 0 = disabled.
	 */
	public function get_auto_block_to_blacklist_threshold() {
		$value = intval( $this->get( 'auto_block_to_blacklist_threshold', 3 ) );
	...
	public function get_trusted_ip_window_seconds() {
		$days = intval( $this->get( 'trusted_ip_window_days', 7 ) );
	 ...
	 *
	 * Starts from the current settings array so keys that are not part of
	 * this submission (for example the retention checkbox on its own form)
	 * are preserved instead of being reset to defaults.
	 *
	 * @since 1.0.0
	...
	public function sanitize( $input ) {
		...
		// API whitelist (paths starting with `/`).
		...
		// Path whitelist (paths starting with `/`).
		...
		if ( isset( $input['log_retention_limit'] ) ) {
			$limit = intval( $input['log_retention_limit'] );
		...
		if ( isset( $input['session_lifetime'] ) ) {
			$lifetime = intval( $input['session_lifetime'] );
		...
		if ( isset( $input['auto_block_trigger_threshold'] ) ) {
			$value = intval( $input['auto_block_trigger_threshold'] );
		...
		if ( isset( $input['auto_block_duration_hours'] ) ) {
			$hours = intval( $input['auto_block_duration_hours'] );
		...
		// Auto-block → blacklist threshold.
		if ( isset( $input['auto_block_to_blacklist_threshold'] ) ) {
			$value = intval( $input['auto_block_to_blacklist_threshold'] );
		...
		if ( isset( $input['trusted_ip_window_days'] ) ) {
			$days = intval( $input['trusted_ip_window_days'] );
