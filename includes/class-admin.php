...
class Admin {
	...
	public function register_hooks() {
		...
		add_filter( 'plugin_action_links_' . JACKLOGU_BASENAME, array( $this, 'action_links' ) );
	...
	public function render_page() {
		...
		$tab  = isset( $_GET['tab'] ) && is_string( $_GET['tab'] ) ? sanitize_key( wp_unslash( $_GET['tab'] ) ) : 'dashboard';
		$tabs = array(
		...
		<div class="wrap">
			...
			<h2 class="nav-tab-wrapper">
				<?php foreach ( $tabs as $key => $label ) : ?>
					<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'jacker-login-guard', 'tab' => $key ), admin_url( 'options-general.php' ) ) ); ?>"
	...
	private function render_settings() {
		$slug      = trim( (string) $this->settings->get( 'slug', JACKLOGU_DEFAULT_SLUG ), '/' );
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						...
						<ul style="margin:6px 0 0 20px;font-size:13px;color:#555;">
							...
							<li><code>192.168.1.10-192.168.1.50</code> — <?php esc_html_e( 'IPv4 range', 'jacker-login-guard' ); ?></li>
							<li><code>192.168.1.100 # My Office</code> — <?php esc_html_e( 'with a comment after # (optional)', 'jacker-login-guard' ); ?></li>
						</ul>
				...
				<tr>
					...
					<td>
						...
						<p class="description"><?php esc_html_e( 'Recommended: 300 entries per log table.', 'jacker-login-guard' ); ?></p>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						...
						<p class="description">
							<?php esc_html_e( 'An IP is blocked only after this many attack attempts within 24 hours. Recommended: 3.', 'jacker-login-guard' ); ?>
						</p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Failed Login Auto-Block', 'jacker-login-guard' ); ?></th>
					<td>
						<input type="number"
							   name="<?php echo esc_attr( JACKLOGU_OPTION_KEY ); ?>[failed_login_block_threshold]"
							   value="<?php echo esc_attr( $this->settings->get_failed_login_block_threshold() ); ?>"
							   min="0" max="100" step="1" class="small-text" />
						<span><?php esc_html_e( 'failed attempts', 'jacker-login-guard' ); ?></span>
						<p class="description">
							<?php esc_html_e( 'An IP is auto-blocked after this many failed login attempts within 24 hours. Set to 0 to disable.', 'jacker-login-guard' ); ?>
							<br>
							<?php esc_html_e( 'Recommended: 5. This applies even when a valid access token is present.', 'jacker-login-guard' ); ?>
						</p>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="number"
							   ...
							   value="<?php echo esc_attr( (int) $this->settings->get( 'auto_block_duration_hours', 24 ) ); ?>"
				...
				<tr>
					...
					<td>
						<input type="number"
							   ...
							   value="<?php echo esc_attr( (int) $this->settings->get( 'trusted_ip_window_days', 7 ) ); ?>"
		...
		<form method="post" enctype="multipart/form-data" style="display:inline-block;margin-left:10px;">
			...
			<input type="file" name="jacklogu_settings_file" accept=".json,application/json" required />
		...
		(function() {
			...
			function updateTable(logins) {
				...
				logins.forEach(function(l, index) {
					...
					var logoutUrl = ACTION_BASE + '&jacklogu_action=force_logout&user_id=' + encodeURIComponent(l.user_id) + '&jacklogu_nonce=' + encodeURIComponent(ACTION_NONCE);
					var whiteUrl  = ACTION_BASE + '&jacklogu_action=whitelist_ip&ip=' + encodeURIComponent(l.ip) + '&jacklogu_nonce=' + encodeURIComponent(ACTION_NONCE);
					var blackUrl  = ACTION_BASE + '&jacklogu_action=blacklist_ip&ip=' + encodeURIComponent(l.ip) + '&jacklogu_nonce=' + encodeURIComponent(ACTION_NONCE);
	...
	private function render_login_rows( $logins, $current_user_id = 0 ) {
		...
		foreach ( $logins as $index => $login ) {
			...
			<tr>
				...
				<td>
					<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=jacker-login-guard&tab=current-login&jacklogu_action=force_logout&user_id=' . (int) $login['user_id'] ), 'jacklogu_event_action', 'jacklogu_nonce' ) ); ?>"
	...
	private function render_access_log() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$filter_status = isset( $_GET['filter_status'] ) && is_string( $_GET['filter_status'] ) ? sanitize_key( wp_unslash( $_GET['filter_status'] ) ) : '';
		...
		<form method="post">
			...
			<p class="description">
				<?php esc_html_e( 'Records access attempts to sensitive resources: login, wp-admin, REST API, XML-RPC, and configuration files. Normal page views are not logged.', 'jacker-login-guard' ); ?>
			</p>
			<p class="description" style="color:#d63638;">
				...
				printf(
					...
					esc_html__( 'An IP is automatically blocked after %d attack attempts within 24 hours. Blocked IPs are listed under Auto Block List.', 'jacker-login-guard' ),
					(int) $this->settings->get_auto_block_trigger_threshold()
				);
				?>
			</p>
		...
		<form method="post">
			...
			<p class="description" style="color:#d63638;">
				...
				printf(
					...
					(int) $this->settings->get( 'auto_block_duration_hours', 24 ),
					(int) $this->settings->get_auto_block_to_blacklist_threshold()
				);
				?>
			</p>
	...
	private function render_row_actions( $ip, $id, $tab, $unblock = false ) {
		...
		if ( $unblock ) {
			$remove_url = wp_nonce_url( $base . '&jacklogu_action=unblock_auto&id=' . (int) $id, 'jacklogu_event_action', 'jacklogu_nonce' );
		} else {
			$remove_url = wp_nonce_url( $base . '&jacklogu_action=delete_event&id=' . (int) $id, 'jacklogu_event_action', 'jacklogu_nonce' );
		}
		?>
	...
	private function render_pagination( $total, $per, $page ) {
		...
		<div class="tablenav bottom">
			<div class="tablenav-pages">
				...
				<?php if ( $page > 1 ) : ?>
					<a class="prev-page button" href="<?php echo esc_url( add_query_arg( 'paged', $page - 1, $base ) ); ?>">&lsaquo;</a>
				<?php endif; ?>
				...
				for ( $i = max( 1, $page - 2 ); $i <= min( $pages, $page + 2 ); $i++ ) :
					...
					<?php else : ?>
						<a class="button" href="<?php echo esc_url( add_query_arg( 'paged', $i, $base ) ); ?>"><?php echo esc_html( $i ); ?></a>
					<?php endif; ?>
				<?php endfor; ?>

				<?php if ( $page < $pages ) : ?>
					<a class="next-page button" href="<?php echo esc_url( add_query_arg( 'paged', $page + 1, $base ) ); ?>">&rsaquo;</a>
				<?php endif; ?>
			</div>
		</div>
		<?php
	...
	public function handle_actions() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( isset( $_GET['jacklogu_action'] ) && is_string( $_GET['jacklogu_action'] ) && 'force_logout' === $_GET['jacklogu_action'] ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$nonce = isset( $_GET['jacklogu_nonce'] ) && is_string( $_GET['jacklogu_nonce'] ) ? sanitize_text_field( wp_unslash( $_GET['jacklogu_nonce'] ) ) : '';
		...
		if ( isset( $_POST['jacklogu_action'] ) && is_string( $_POST['jacklogu_action'] ) ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$action = isset( $_GET['jacklogu_action'] ) && is_string( $_GET['jacklogu_action'] ) ? sanitize_text_field( wp_unslash( $_GET['jacklogu_action'] ) ) : '';
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$tab      = isset( $_GET['tab'] ) && is_string( $_GET['tab'] ) ? sanitize_key( wp_unslash( $_GET['tab'] ) ) : 'login-events';
		$redirect = admin_url( 'admin.php?page=jacker-login-guard&tab=' . $tab );

		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$ip = isset( $_GET['ip'] ) && is_string( $_GET['ip'] ) ? sanitize_text_field( wp_unslash( $_GET['ip'] ) ) : '';
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	private function handle_bulk( $action ) {
		...
		$nonce = isset( $_POST['jacklogu_nonce'] ) && is_string( $_POST['jacklogu_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['jacklogu_nonce'] ) ) : '';

		if ( ! wp_verify_nonce( $nonce, 'jacklogu_event_action' ) ) {
			wp_die( esc_html__( 'Security check failed.', 'jacker-login-guard' ) );
		}

		$tab      = isset( $_POST['jacklogu_tab'] ) && is_string( $_POST['jacklogu_tab'] ) ? sanitize_key( wp_unslash( $_POST['jacklogu_tab'] ) ) : 'login-events';
		$redirect = admin_url( 'admin.php?page=jacker-login-guard&tab=' . $tab );

		if ( 'access-log' === $tab && isset( $_POST['filter_status'] ) && is_string( $_POST['filter_status'] ) ) {
		...
		$bulk = isset( $_POST['bulk_action'] ) && is_string( $_POST['bulk_action'] ) ? sanitize_key( wp_unslash( $_POST['bulk_action'] ) ) : '';
	 ...
	 *
	 * Uses a single batched query instead of N+1.
	 *
	 * @since 1.0.0
	...
	private function collect_ips_for_tab( $tab, $ids, $login_table, $access_table, $auto_block_table ) {
		...
		// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$query = $wpdb->prepare(
			"SELECT user_ip FROM {$table} WHERE id IN ({$placeholders})",
			$ids
		);
	 ...
	 *
	 * Re-reads fresh from the database to reduce race conditions.
	 *
	 * @since 1.0.0
	 * @param string $ip  IP address.
	...
	private function add_to_list( $ip, $key ) {
		$fresh   = get_option( JACKLOGU_OPTION_KEY, array() );
		$current = isset( $fresh[ $key ] ) ? $fresh[ $key ] : '';
	...
	private function remove_auto_block( $ip ) {
		...
		// Clear the "not blocked" cache so the change takes effect immediately.
		delete_transient( 'jacklogu_not_blocked_' . md5( $ip ) );
	}
	...
	public function handle_reset() {
		if ( ! isset( $_POST['jacklogu_action'] ) || ! is_string( $_POST['jacklogu_action'] ) || 'reset_settings_post' !== $_POST['jacklogu_action'] ) {
			return;
		}
		...
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
		if ( ! isset( $_POST['jacklogu_reset_nonce'] ) || ! is_string( $_POST['jacklogu_reset_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['jacklogu_reset_nonce'] ) ), 'jacklogu_reset_action' ) ) {
			wp_die( esc_html__( 'Security check failed.', 'jacker-login-guard' ) );
		}
	 ...
	 *
	 * Export is triggered via POST; import validates file size and MIME.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function handle_import_export() {
		...
		// Export via POST.
		if ( isset( $_POST['jacklogu_action'] ) && is_string( $_POST['jacklogu_action'] ) && 'export_settings' === $_POST['jacklogu_action'] ) {
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			$nonce = isset( $_POST['jacklogu_export_nonce'] ) && is_string( $_POST['jacklogu_export_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['jacklogu_export_nonce'] ) ) : '';
		...
		// Import via POST.
		if ( isset( $_POST['jacklogu_action'] ) && is_string( $_POST['jacklogu_action'] ) && 'import_settings' === $_POST['jacklogu_action'] ) {
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			$nonce = isset( $_POST['jacklogu_import_nonce'] ) && is_string( $_POST['jacklogu_import_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['jacklogu_import_nonce'] ) ) : '';

			if ( ! wp_verify_nonce( $nonce, 'jacklogu_import_action' ) ) {
				wp_die( esc_html__( 'Security check failed.', 'jacker-login-guard' ) );
			}

			if ( ! isset( $_FILES['jacklogu_settings_file']['error'] ) || UPLOAD_ERR_OK !== (int) $_FILES['jacklogu_settings_file']['error'] ) {
			...
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			$file_size = isset( $_FILES['jacklogu_settings_file']['size'] ) ? (int) $_FILES['jacklogu_settings_file']['size'] : 0;

			if ( $file_size > 1024 * 1024 ) {
				add_settings_error( JACKLOGU_OPTION_KEY, 'import_error', __( 'File too large (max 1 MB).', 'jacker-login-guard' ) );
				return;
			}
			...
			$merged    = wp_parse_args( $data['settings'], Settings::get_defaults() );
			$sanitized = $this->settings->sanitize( $merged );
	 ...
	 *
	 * Only shown on the plugin's own settings screen.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function show_settings_errors() {
		...
		if ( ! $screen || 'settings_page_jacker-login-guard' !== $screen->id ) {
			return;
		}
