...
class Plugin {
	...
	public function redirect_after_logout( $redirect_to, $requested_redirect_to, $user ) {
		...
		$slug = trim( (string) Settings::get_instance()->get( 'slug', JACKLOGU_DEFAULT_SLUG ), '/' );
	 ...
	 *
	 * Only output when a custom session lifetime is configured.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function output_heartbeat_script() {
		...
		if ( ! Settings::get_instance()->get_session_lifetime() ) {
			return;
		}
	...
	/**
	 * Redirect author links to the homepage when enumeration is blocked.
	 *
	 * @since 1.0.0
