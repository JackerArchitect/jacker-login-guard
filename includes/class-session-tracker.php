...
class SessionTracker {
	 ...
	 * @var   int
	 */
	const ONLINE_TTL = 1800;

	/**
	 * Minimum seconds between last-activity writes.
	 *
	 * @since 1.0.0
	 * @var   int
	 */
	...
	public function update_activity() {
		...
		if ( $lifetime > 0 ) {
			...
			if ( ! empty( $token ) ) {
				...
				$key        = 'jacklogu_activity_' . $token_hash;

				$activity = get_transient( $key );

				if ( false === $activity ) {
					set_transient( $key, $now, $lifetime );
				} else {
	...
	/**
	 * Force-logout every session belonging to a user and clear markers.
	 *
	 * @since 1.0.0
	 * @param int $user_id User ID.
