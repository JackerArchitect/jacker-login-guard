...
class AccessControl {
	...
	/**
	 * Admin AJAX actions that are never audited.
	 *
	 * @since 1.0.0
	 * @var   string[]
	 */
	...
	/**
	 * Substrings that mark an action as state-changing.
	 *
	 * @since 1.0.0
	 * @var   string[]
	 */
	...
	/**
	 * Admin scripts always audited on POST.
	 *
	 * @since 1.0.0
	 * @var   string[]
	 */
	 ...
	 * @var   int
	 */
	const ADMIN_AUDIT_DEDUP_TTL = 900;
	...
	public function register_hooks() {
		...
		// Invalidate cached page checks when posts change.
		add_action( 'save_post', array( $this, 'flush_page_check_cache' ) );
		add_action( 'deleted_post', array( $this, 'flush_page_check_cache' ) );
		add_action( 'trashed_post', array( $this, 'flush_page_check_cache' ) );
	}

	/**
	 * Flush the page-existence cache.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function flush_page_check_cache() {
		...
		foreach ( array_merge( array( $slug ), self::$sensitive_paths ) as $s ) {
			wp_cache_delete( 'jacklogu_page_check_' . md5( $s ), 'jackerloginguard' );
		}
	}
	...
	private function maybe_log_admin_operation() {
		...
		if ( ! empty( $action ) && 0 === strpos( $action, 'jacklogu_' ) ) {
			return;
		}
	...
	private function handle_login_access( $client_ip ) {
		...
		if ( $this->is_admin_request( $request_uri ) && false === strpos( $request_uri, 'admin-ajax.php' ) ) {
	 ...
	 *
	 * Positive results are not cached (a block must take effect immediately).
	 * Negative results are cached for 5 minutes to reduce database load.
	 *
	 * @since 1.0.0
	 * @param string $ip IP address.
	 * @return bool
	...
	private static function is_auto_blocked( $ip ) {
		...
		$cache_key = 'jacklogu_not_blocked_' . md5( $ip );
	 ...
	 *
	 * Only GET requests qualify. POST requests (login, passkey, 2FA) must
	 * present a valid access token instead.
	 *
	 * @since 1.0.0
	 * @return bool
	 */
	 ...
	 *
	 * Detects both `/wp-json/...` and `?rest_route=...` forms.
	 *
	 * @since 1.0.0
	 * @return bool
	 */
	private function is_rest_enumeration() {
		...
		// 1) Explicit ?rest_route= parameter.
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		if ( '' === $route ) {
			...
			if ( 0 === strpos( $path, 'wp-json/' ) ) {
				...
				$route = '/' . substr( $path, strpos( $path, '/wp-json/' ) + strlen( '/wp-json/' ) );
			}
		}
	...
	private function is_admin_request( $path ) {
		...
		$admin = trim( (string) wp_parse_url( admin_url(), PHP_URL_PATH ), '/' );

		if ( ! empty( $admin ) && ( $path === $admin || 0 === strpos( $path, $admin . '/' ) ) ) {
			return true;
		}
